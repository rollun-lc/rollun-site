...
Разработчик получает рабочий каркас проекта (Next.js 16 + Payload в одном процессе) с единой дизайн-системой и **зафиксированным критерием пиксель-приёмки (SM-1)**, а посетитель на **любой** из 6 страниц видит консистентную фирменную шапку, футер и мобильную навигацию, пиксель-в-пиксель по Handoff. Реализует FR-1.
**FRs covered:** FR-1
**NFR/Additional:** NFR-1 (одна DS), NFR-2 (брейкпоинты/768px), NFR-3 (вендоринг шрифтов/либ), SM-1 (критерий приёмки пикселя), AD-1/2/3/12/13 (Islands, `@theme`, две композиции, один процесс, дизайн-источник), UX-DR1–6.
*Enables:* всё остальное. Standalone: полностью рабочая оболочка + каркас + процедура приёмки пикселя.
...
### Epic 7: CMS — паспорт компании и управляемый контент страниц (Фаза 2)
Нетехнический менеджер меняет паспорт компании, картинки и тексты страниц через админку Payload без разработчика и без риска сломать вёрстку; изменения появляются на проде без пересборки. Реализует FR-12 и текстово-медийную часть FR-13, UJ-3, SM-3.
**FRs covered:** FR-12, FR-13 (слоты текста/медиа/паспорта)
...
*Standalone:* CMS-контур Фазы 2 (паспорт + медиа + текстовые слоты страниц + ревалидация).

### Epic 8: CMS — каталожные данные из CMS и роли (Фаза 3)
Менеджер наполняет Catalog и Our Brands из CMS (товары/бренды/магазины) с разграничением прав доступа; офферы остаются рантайм-деривацией. Завершает FR-13 для товарных данных. Реализует FR-13 (данные каталога), UJ-3, SM-3.
**FRs covered:** FR-13 (каталожные коллекции)
**NFR/Additional:** AD-5 (`Brands` ≠ `BrandsContent`), AD-6 (текучесть), AD-9 (офферы — не поле, sku/externalId-шов), access control Payload (роли admin/manager).
*Standalone:* CMS-контур Фазы 3 (Products/Brands/Shops + роли). *(Фаза 4 — авто-подтяг товаров из фида и Posts — вынесена за пределы этой разбивки как отдельный будущий эпик.)*
...
**And** приложение упаковано в Docker-контейнер (`Dockerfile`, один Node 20+ процесс — сайт + `/admin` + local API, AD-12) для self-host на инфре Rollun; контейнер собирается и стартует локально с заданными env `[ASSUMPTION: docker-compose с Postgres, TLS/reverse-proxy и окружения (staging/preview под пиксель-приёмку SM-1) — уточняются владельцем перед деплоем Фазы 1, см. Architecture Deferral «Деплой-конвейер»]`
...
As a владелец продукта (и разработчик),
I want однозначный, повторяемый способ подтвердить пиксель-в-пиксель по Handoff,
So that «готово» для каждой страницы определяется формально, а не на глаз (защищает главный критерий SM-1 и контр-метрику SM-C1).

**Acceptance Criteria:**

**Given** приоритет №1 — пиксель-в-пиксель по Handoff (SM-1), а дизайн — источник истины (AD-13)
**When** зафиксирован критерий приёмки
**Then** в репозитории есть **чеклист приёмки на каждую из 6 страниц** с явным перечнем целевых брейкпоинтов desktop (постраничные наборы вплоть до 1280 — из файла прототипа, NFR-2/AD-3) + mobile-композиция (≤768px, letterbox-шелл 440px); каждый брейкпоинт — отдельный пункт «визуал совпал с отрисованным прототипом»
**And** для каждой страницы указан канонический Handoff-референс (`*.html` + `* Mobile.html` по таблице addendum §H), против которого матчится визуал
**And** поднято **preview/staging-окружение** (или локальная preview-команда), где собранную страницу можно открыть на каждом целевом брейкпоинте для ревью SM-1 `[ASSUMPTION: конкретное staging-окружение на инфре Rollun уточняет владелец — см. Architecture Deferral «Деплой-конвейер и окружения»; до этого достаточно локального preview]`
**And** чеклист учитывает воспроизводимые дефекты дизайна (AD-13, UX-DR24) как «ожидаемо», а не как расхождение
**And** приёмка страницы в Epic 3–6 ссылается на этот чеклист (страница не считается Done, пока все её брейкпоинт-пункты не отмечены)
**And** *(опционально, не блокирует)* автоматический visual-regression/pixel-diff — вынесен в будущее (Architecture Deferred); этот критерий работает и на ручном ревью по чеклисту
...
Нетехнический менеджер меняет паспорт, картинки и тексты страниц через админку без разработчика и без риска сломать вёрстку; изменения — на проде без пересборки. Реализует FR-12, текстово-медийную часть FR-13, UJ-3, SM-3. *(Товары/бренды/магазины из CMS — в Epic 8, Фаза 3.)*
...
Менеджер наполняет Catalog и Our Brands из CMS (товары/бренды/магазины) с разграничением прав; офферы остаются рантайм-деривацией. Завершает FR-13 для товарных данных. Реализует FR-13 (данные каталога), UJ-3, SM-3.
