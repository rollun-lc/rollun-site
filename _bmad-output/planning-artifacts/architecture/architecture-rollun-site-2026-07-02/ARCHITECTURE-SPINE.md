...
- **Rule:** страница и секция — Server Components. `'use client'` только на листовом островке интерактива из фиксированного перечня: мобильное шасси (drawer + scroll-lock, header shrink-on-scroll, footer-аккордеоны, reveal-on-scroll IntersectionObserver — из `mobile.js`), product-line слайдер (Home), count-up счётчики, D3-карта US Presence (About), Catalog card-slider + quick-view + marquee-hover-pause, `ContactForm` (все режимы), map-табы (Contact), lightbox сертификата (Our Brands mobile). Островок получает контент **пропсами** и контент не фетчит.
...
- **Rule:** все DS-токены прототипов (цвета, типошкала, spacing, radii, shadows, easing) живут в одном `@theme`-блоке (Tailwind v4, CSS-first — эмит и как `:root`-переменные, и как утилиты). Семейства **Poppins/Roboto/Karla** (+ **Caveat** на About), оранж **`#EF7F1A`** — единственный. Поверхностно-специфичные значения (mobile `--bg:#D2D2D2`, `--shell-w:440px`) — **scoped-переопределения тех же токен-имён**, не новые имена. `colors_and_type.css` игнорируется. Компонент не пишет сырой цвет/шрифт/spacing-литерал, существующий как токен.
...
- **Rule:** каждая страница поставляет **две композиции** (desktop | mobile) над общими DS и контентом; переключение на едином пороге **~768px**; desktop держит свои внутренние брейкпоинты (560–1100). `[ASSUMPTION механизм: обе композиции в DOM, переключение CSS-медиа на 768px, обе SSR — чтобы избежать hydration-mismatch от server-side UA-сниффинга и cache-vary по UA; кандидат на пересмотр, если вес DOM/дублирование медиа станет критичным.]`
...
- **Rule:** каждый текст/ассет классифицируется: 🔴 живое (картинки, товары) → CMS · 🟡 правимый текст живых блоков → CMS · 🟢 паспорт (тел/адрес/соц/часы) → `SiteSettings` · ⚫ гвоздями (вёрстка, структура, юр/статичный текст, микрокопи) → код. Слотами становятся только 🔴🟡🟢; ⚫ остаётся в коде.
...
- **Rule:** каждая страница — чистая функция типизированного контент-объекта. Фаза 1 подаёт **статический инстанс** этого типа (`content/*`); Фазы 2–3 подают тот же тип из Payload Global. Подключение CMS меняет **источник, не разметку** → пиксель между фазами не меняется. Граница компонента уже совпадает с будущим слотом.
...
- **Rule:** `offers` вычисляются одной общей функцией `buildOffers` из продуктовой линии (Health → Amazon/eBay; Automotive → Amazon/eBay/Walmart) + репрезентативная цена; это **не** поле Payload. На `Products` с Фазы 1 зарезервированы `sku`/`externalId`; фид-перезаписываемые поля (цена, картинки, наличие) изолированы под Фазу 4.
...
- **Rule:** страницы статически рендерятся; правка контента триггерит `revalidatePath`/`revalidateTag` из `afterChange`-хуков Payload (согласовано с явной моделью Cache Components в Next 16). Per-page ad-hoc кэша нет. Фаза 1 (хардкод-контент) — полностью статична.
